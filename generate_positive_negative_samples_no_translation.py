...
for coupon_type in ['positive', 'negative']:
    try:
        print(f'Attempting to read existing {coupon_type}_coupons_train.csv...')
        examples_dict[coupon_type] = pd.read_csv(f'{coupon_type}_coupons_train.csv')
    except:
        print(f'File not found. Creating {coupon_type} examples...')
...
# Permutation of user-coupon interactions from test coupons
